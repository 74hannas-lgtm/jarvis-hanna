...
Je m'appelle Hanna et je vis à Annemasse, en France. Je suis aide-soignante à Genève, et en parallèle je développe ma marque de bijoux de corail "Majdi Corail" sur TikTok et Instagram. J'ai créé un avatar influenceuse IA nommée Maya pour présenter mes produits et parler du corail. Mon ambition est de lancer une boutique Shopify et de vivre un jour de mon contenu et de mes ventes.

Mes objectifs prioritaires actuels sont d'atteindre 10 000 abonnés sur TikTok, de lancer ma boutique Shopify et de publier du contenu régulier avec Maya.

À long terme, je veux vivre de ma boutique et de mon contenu, sans dépendre de mon travail d'aide-soignante.

Le domaine où j'ai besoin du plus d'aide en ce moment : stratégie réseaux sociaux, croissance des abonnés et création de contenu.
...
├── livrables/                   # Outputs produits par Claude
│   ├── contenu/                 # Scripts, captions, posts TikTok/Instagram
│   ├── maya/                    # Tout ce qui concerne l'avatar Maya
│   ├── boutique/                # Fiches produits, textes Shopify
│   └── strategie/               # Plans, analyses, recommandations
├── secrets/                     # Clés d'API (ne jamais partager)
│   ├── keys.env.example         # Modèle vide des clés
│   └── keys.env                 # Tes vraies clés (à créer, ne pas partager)
├── .claude/
│   ├── commands/
...
| `secrets/` | Clés d'API et secrets, ne jamais partager |
...
### Règle d'or : où ranger les fichiers

- **Tu fournis un document** (PDF, export, note, capture d'écran) → `context/import/`
- **Claude produit quelque chose pour toi** (script, plan, fiche produit, analyse) → `livrables/[sous-dossier-adapté]/`

### Règle secrets

- Ne jamais coller une clé d'API dans le chat. Dis à Claude : "ma clé est dans secrets/keys.env"
- Ne jamais partager le fichier `secrets/keys.env`
- Toujours utiliser `secrets/keys.env.example` comme modèle pour documenter une nouvelle clé
